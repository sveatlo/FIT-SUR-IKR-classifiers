...
def mel_filter_bank(nfft, nbands, fs, fstart=0, fend=None):
    ...
    mfb = np.zeros((int(nfft / 2 + 1), int(nbands)))
...
def wav16khz2mfcc(dir_name):
    ...
    for f in glob(dir_name + '/*.wav'):
        ...
        assert(rate == 16000)
        features[f] = mfcc(s[32000:], 400, 240, 512, 16000, 23, 13)
        # print("{} len is {}, # of coeffs is {} -> ratio {}".format(f, len(s), len(features[f]), len(s) / len(features[f])))
    return features
